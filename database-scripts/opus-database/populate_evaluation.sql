...
INSERT INTO note.evaluation (evaluation_type_id, eg_id, validity_start, label, short_description, user_id)
	...
	FROM note.evaluation_type et, note.educationnal_goal eg
	WHERE et.label = 'Somatif' AND eg.label = 'gis3');
SELECT note.create_rubric_and_criterion((SELECT last_value FROM note.evaluation_evaluation_id_seq)::integer, 'gis3_app1_rapport_q1', 'Question 1', 'gif310-1', 40);
SELECT note.create_rubric_and_criterion((SELECT last_value FROM note.evaluation_evaluation_id_seq)::integer, 'gis3_app1_rapport_q2', 'Question 2', 'gif310-2', 140);
SELECT note.create_rubric_and_criterion((SELECT last_value FROM note.evaluation_evaluation_id_seq)::integer, 'gis3_app1_rapport_q3', 'Question 3', 'gif362-1', 32);
SELECT note.create_rubric_and_criterion((SELECT last_value FROM note.evaluation_evaluation_id_seq)::integer, 'gis3_app1_rapport_q4', 'Question 4', 'gif362-2', 32);
