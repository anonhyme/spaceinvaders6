...
INSERT INTO note.educationnal_goal(eg_id, label, short_description, description, administrative_value, validity_start, validity_end, registration, user_id, eg_type_id) VALUES (DEFAULT, 'g1s2', 'Session 2', '', 0, now(), DEFAULT, now(), 1, 2); INSERT INTO note.educationnal_goal_hierarchy (SELECT a.eg_id, b.eg_id, now(), 1 FROM note.educationnal_goal a JOIN ( select last_value as eg_id from note.educationnal_goal_eg_id_seq ) b ON 1=1 WHERE a.label = 'GI'); 
...
*/
INSERT INTO note.educationnal_goal_instance(timespan_id, eg_id, user_id) (SELECT t.timespan_id, eg.eg_id, 1 FROM note.timespan t, note.educationnal_goal eg WHERE t.label = 'A14' AND eg.label = 'geis1sn');
...
INSERT INTO note.assigned_group (timespan_id, eg_id, privilege_id, group_id, user_id) 
	(SELECT egi.timespan_id, egi.eg_id, p.privilege_id, g.group_id, 1
		...
		WHERE egi.eg_id = eg.eg_id AND egi.timespan_id = t.timespan_id AND eg.label = 'geis1sn' AND t.label = 'A12' AND g.label = 'GI58' AND p.label = 'Accès membre');

/**
	Create an evaluation that represent a final exam, lab report, etc
*/
 
INSERT INTO note.evaluation (evaluation_type_id, eg_id, label, user_id)
